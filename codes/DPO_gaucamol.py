...
from utils import set_seed, sample_SMILES, calc_fingerprints
...
class DPO_goal_directed_generator(GoalDirectedGenerator):
    ...
    def __init__(self, logger, configs):
        self.load=False             #如果因为异常情况出现中断，可以将其调为True继续训练（会继承GPTmodel参数和optimizer参数以及memory）
        self.load_memory=not configs.first_course
    ...
    def sample(self,agent,step,ratio=2):
        ...
        if (step<100 and step%10==0) or (step<200 and step%5==0) or (step<300 and step%2==0) or step>=300:
            ...
            scores = self.scoring_function.score_list(samples)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--first_course', type=bool, default=True)
